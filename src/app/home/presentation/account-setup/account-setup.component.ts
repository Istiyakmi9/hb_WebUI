...
export class AccountSetupComponent implements OnInit {
  ...
  getJobsandLocation() {
    this.isLoading = true;
    this.http
      .get(`user/getJobandLocation/${this.selectedJobType.CategoryId}`)
  ...
  selectJobTitle(e: any) {
    ...
    if (index < 0) this.selectedJobTitle.push(e);
    else this.selectedJobTitle.splice(index, 1);
  }

  selectLocation(e: any) {
    let index = this.selectedLocation.findIndex((x) => x.value == e.value);
    if (index < 0) this.selectedLocation.push(e);
    else this.selectedLocation.splice(index, 1);
  }
    ...
    let value = {
      ...
      CategoryTypeIds:
        this.selectedJobTitle.length > 0
          ? this.selectedJobTitle.map((x) => x.value)
          : [],
      JobLocationIds:
        this.selectedLocation.length > 0
          ? this.selectedLocation.map((x) => x.value)
          : [],
    };
    this.http
      .post('user/addJobandLocation', value)
      .then((res: ResponseModel) => {
        if (res.ResponseBody) {
          this.nav.navigate(Index, null);
          this.isLoading = false;
